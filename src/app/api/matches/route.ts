...
export async function POST(req: Request) {
  try {
    ...
    const existingSet = new Set(
      (existingMatches || []).map(
        (m: any) => `${m.requirement_id}_${m.profile_id}`,
      ),
    );
      ...
      for (const prof of profiles) {
        ...
        if (intersection.length > 0) {
          const score = Math.round(
            (intersection.length / reqSkills.length) * 100,
          );
    ...
    if (matchesToInsert.length > 0) {
      ...
      if (matchError)
        throw new Error("Match batch insert failed: " + matchError.message);
    }

    return NextResponse.json({
      success: true,
      matches_found: matchesToInsert.length,
      details: matchesToInsert.map((m) => ({
